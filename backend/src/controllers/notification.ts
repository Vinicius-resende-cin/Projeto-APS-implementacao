...
export class ConcreteNotifier implements Notifier {
    ...
    generateAwsSignature(secretKey: string, dateStamp: string, region: string, serviceName: string, stringToSign: string): string {
    ...
    send() {
        ...
        const signature = this.generateAwsSignature(this.secretAccessKey, dateStamp, region, 'mobiletargeting', stringToSign);
...
export class ConcreteDecoratorEmail extends Decorator {
    send(title:string, textMessage:string, htmlMessage:string, addresses:string[], addressesData:object): void {
        ...
        this.notifier.addresses = {...this.notifier.addresses, ...(addresses.reduce((obj, address) => {
...
export function orderStatusChange(user:User, order:Order){  
    ...
    const addressesData = {[user.email]: {
        "user.name": [user.name],
        "order.id": [order.id],
